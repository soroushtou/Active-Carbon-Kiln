...
//if(temperature - temperature_set >= -30 && temperature - temperature_set <= 30){
  ...
  if(analyzer_counter == 31){digitalWrite(Air_valve,HIGH);digitalWrite(Gas_valve,LOW);digitalWrite(Gas_pump,LOW);}
  if(analyzer_counter == 61){digitalWrite(Gas_valve,HIGH);digitalWrite(Air_valve,LOW);digitalWrite(Steam_pump,LOW);set_steam_mode(LOW);}
  if(analyzer_counter == 151){digitalWrite(Gas_pump,HIGH);digitalWrite(Air_valve,HIGH);}
...
void RPM_measure(void){
  ...
  time_prev = millis();
  //if(time_diff>400){
  feeder_RPM = 60000.0 / (float)time_diff;
