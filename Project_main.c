...
timer_counter ++;
// Place your code here
if(timer_counter == 51){    //6.7sec
scale_total = scale_total / measure_counter;
//OCR2 = (int)fuzzy[x][y]*2.55;
//itoa(scale_total,str1);
sprintf(buffer4,"%4.1fg",scale_total); 
sprintf(buffer,"%4.1fg %4.1fg %4.1fg %4.1fg ",scale,scale1,scale2,scale_total);//sprintf(buffer," %ld ",Weight_Shiwu);//sprintf(buffer," %1.3f --> Gram",scale);
lcd_clear();  
lcd_gotoxy(0,0);
lcd_puts(buffer); 
//delay_ms(10);
...
void get_scale(void)
{
    ...
    sort_array(scale_array,15);
    scale = scale_array[7]*1.2874;
    sort_array(scale1_array,15);
    scale1 = scale1_array[7]*1.1929;
    sort_array(scale2_array,15);
    scale2 = scale2_array[7]*1.1039;
    }
