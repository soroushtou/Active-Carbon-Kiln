...
int feeder_RPM_set = 20;   // 0 - 34 RPM
...
int temperature,temperature_diff ,stepper_counter,analyzer_counter,floater_counter,process_startup_counter,stepper_position,stepper_prev,flame_mode,scale_speed;
...
stepper_counter ++;
  ...
  if(stepper_counter > 60){
    ...
    if( temperature_diff < -5 && stepper_position > 48) {digitalWrite(Flame_stepper2,LOW);digitalWrite(Flame_stepper1,HIGH);}
  else if(temperature_diff  > 5){digitalWrite(Flame_stepper2,HIGH);digitalWrite(Flame_stepper1,LOW);}
  stepper_counter = 0;
  }
