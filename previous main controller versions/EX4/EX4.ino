...
float R0_MQ7_temp;
float volt_MQ7; // xxxxxxGet ratio RS_GAS/RS_air         Get Sensor Volt
float MQ7_ppm;
float R0_MQ2;  // Get the value of R0 via in H2
float R0_MQ2_temp;
float volt_MQ2; // xxxxxGet ratio RS_GAS/RS_air         Get Sensor Volt
float MQ2_ppm;
int count_display = 0;
unsigned int menu = 0;
...
void display_lcd(void){
  ...
  if(save_r0 == true){
    ...
    switch(menu){
      ...
      lcd.setCursor(0,1);
      lcd.print(R0_MQ2_temp);
      Serial.println("New R0 for MQ2:");
      Serial.println(R0_MQ2_temp);
      break;
      
      case 1:
      lcd.print("New R0 for MQ7:");
      lcd.setCursor(0,1);
      lcd.print(R0_MQ7_temp);
      Serial.println("New R0 for MQ7:");
      Serial.println(R0_MQ7_temp);
      break;
  }
}
...
void get_R0_MQ7(void){
  ...
  // Get a average data by testing 100 times
    ...
    R0_MQ7_temp = RS_air/9.8; // The ratio of RS/R0 is 9.8 in a clear air from Graph (Found using WebPlotDigitizer)
}
...
void get_R0_MQ2(void){
  ...
  // Get a average data by testing 100 times
    ...
    sensor_volt = sensorValue/1024*5.0;
    RS_air = (5.0-sensor_volt)/sensor_volt; // omit * RL
    R0_MQ2_temp = RS_air/9.8; // The ratio of RS/R0 is 9.8 in a clear air from Graph (Found using WebPlotDigitizer)
}
