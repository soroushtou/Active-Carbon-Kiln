...
int temperature_set = 500;    // 0 to 1023 C
int steam_mode = 2;         // 1 , 2 , 3
...
if(temperature_counter >= 180){
  ...
  if(temperature_diff >= 20 && temperature_diff < 60)myStepper.step(36 - stepper_position);     //          goes to 64deg
  if(temperature_diff < 20 && temperature_diff > -20);                                   //does not change the flame_mode
  if(temperature_diff <= -20 && temperature_diff > -60)myStepper.step(30 - stepper_position);   //          goes to 53deg
  if(temperature_diff <= -60 && temperature_diff > -100)myStepper.step(22 - stepper_position);  //          goes to 40deg
  ...
  if(digitalRead(Floater)==LOW){
    floater_counter = 10;
    digitalWrite(Water_inlet,LOW);
  }
  if(floater_counter > 0) floater_counter-- ; 
  else digitalWrite(Water_inlet,HIGH);
}
