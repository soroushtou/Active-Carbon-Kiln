...
int feeder_on_time_set = 3; // 0 - 10 s
int feeder_RPM_set = 20;   // this parameter is cte , dont change , 0 - 34 RPM
float kiln_RPM_set = 7;     // 0.00 - 12.00 RPM
int temperature_set = 385;    // 0 to 1023 C
int steam_mode = 3;         // 1 , 2 , 3
...
int temperature,temperature_diff ,stepper_counter,analyzer_counter,floater_counter,process_startup_counter,stepper_position,stepper_prev,flame_mode,scale_speed,feeder_on_time_counter,c;
bool temperature_condition;
boolean prevstate;
unsigned long time_prev,time_prev2,time_prev3,time_prev4,time_diff;
...
//if(process_startup_counter == 2){ set_steam_mode(LOW);digitalWrite(Steam_pump,LOW);}
if(process_startup_counter == 3){ digitalWrite(Output_cooling_rotation,LOW);digitalWrite(Output_cooling_water,LOW);TCCR5B = 0;/*Timer5 Disable*/}
}
...
cli();
if(temperature_condition==1 && feeder_on_time_counter < feeder_on_time_set){
...
stepper_counter ++;
  ...
  if(analyzer_counter == 31){digitalWrite(Air_valve,HIGH);delay(20);digitalWrite(Gas_valve,LOW);delay(20);digitalWrite(Gas_pump,LOW);}
  if(analyzer_counter == 61){digitalWrite(Gas_valve,HIGH);delay(20);digitalWrite(Air_valve,LOW);delay(20);digitalWrite(Steam_pump,LOW);delay(20);set_steam_mode(LOW);}  /////////////////////////be in khat mirese mitereke
  if(analyzer_counter == 151){digitalWrite(Gas_pump,HIGH);delay(20);digitalWrite(Air_valve,HIGH);}
  if(analyzer_counter == 240){analyzer_counter=0;}
}


  if(stepper_counter > 60){
    //digitalWrite(Flame_stepper2,HIGH);digitalWrite(Flame_stepper1,LOW);
    if( temperature_diff < -5 && stepper_position > 48) {digitalWrite(Flame_stepper2,LOW);digitalWrite(Flame_stepper1,HIGH);stepper_prev = stepper_position;}
  else if(temperature_diff  > 5){digitalWrite(Flame_stepper2,HIGH);digitalWrite(Flame_stepper1,LOW);stepper_prev = stepper_position;}
  stepper_counter = 0;
  }
  
    feeder_on_time_counter++;
    if(feeder_on_time_counter < feeder_on_time_set){
      OCR4BL = c; 
      //TCCR3B |= (1 << WGM32); // enable timer compare interrupt
      } 
    else{
      OCR4BL = 0; 
      //TCCR3B &= (~(1 << WGM32)); // disable timer compare interrupt
      }
    if(feeder_on_time_counter == 10){feeder_on_time_counter = 0;}
...
Serial.print(",");
Serial.print(steam_mode);
Serial.print(",");
Serial.println(feeder_on_time_set);
...
TCCR5B |= (1 << WGM52); // Start Process Startup Timer (Timer 5) 
...
void loop() {
  ...
  get_stepper_position();
  if(stepper_position < 48 && temperature_diff < -5){digitalWrite(Flame_stepper1,LOW);digitalWrite(Flame_stepper2,LOW);}
  if(temperature_diff < 5 && temperature_diff > -5){digitalWrite(Flame_stepper1,LOW);digitalWrite(Flame_stepper2,LOW);}
  if(stepper_position - stepper_prev < -3){digitalWrite(Flame_stepper1,LOW);digitalWrite(Flame_stepper2,LOW);}
  if(stepper_position - stepper_prev > 3){digitalWrite(Flame_stepper1,LOW);digitalWrite(Flame_stepper2,LOW);}
